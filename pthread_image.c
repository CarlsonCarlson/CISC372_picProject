...
void *worker(void *vargs) {
    ...
    int pix, bit;
    struct arg_input *parg = (struct arg_input *) vargs;
...
void convolute(Image *srcImage, Image *destImage, Matrix algorithm) {
    ...
    for (row = 0; row < srcImage->height; row++) {
        ...
        struct arg_input *arg = (struct arg_input *) malloc(sizeof(struct arg_input));
        arg->row = row;
        arg->srcImage = srcImage;
        arg->destImage = destImage;
        for (i = 0; i < 3; i++) {
            for (j = 0; j < 3; j++) {
                arg->algorithm[i][j] = algorithm[i][j];
            }
        }
        // create thread
        pthread_create(pids + row, NULL, worker, arg);
    }
